...
class NaverNewsCrawlServiceTest {
    ...
    void crawl_newNews_addsAutoPostMetadata() {
        given(naverNewsConfig.getClientId()).willReturn("client-id");
        given(naverNewsConfig.getClientSecret()).willReturn("client-secret");
        ...
        ArgumentCaptor<List<InternalDocumentPayload>> captor = ArgumentCaptor.forClass(List.class);
        verify(internalCrawlClient).sendDocuments(captor.capture());
