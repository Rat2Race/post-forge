...
DB ownership은 [DB Schema Ownership](../database/schema-ownership.md)에 둔다.
...
- JPA auditing, request logging, exception response base 같은 Spring infrastructure는 `support`로 분리해 기능 모듈에 섞이지 않게 한다.
...
- 분산 transaction을 피하기 위한 event/idempotency 설계가 필요하다.
...
| Spring infrastructure 분리 | `support/src/main/java/dev/iamrat/support` | 공통 web/persistence 보조 코드가 기능 모듈에 섞이지 않아 서비스별 starter/config로 분해하기 쉽다. |
...
- transaction 경계를 넘는 흐름의 event/idempotency 설계
...
다만 DB ownership, event, service observability 같은 분산 시스템 문제는 분리 시점에 별도 설계가 필요하다고 명시했습니다.
