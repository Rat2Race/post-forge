...
- 각 기능 모듈은 직접 사용하는 기술 의존성을 직접 선언한다.
...
현재 구조는 MSA 자체가 아니다. 분리 시점에는 다음 설계가 추가로 필요하다.
...
- `messaging` 모듈과 outbox 경계가 추가됐다. 현재 그래프는 [Module Dependency Policy](../architecture/module-dependencies.md), 전달 결정은 [이벤트 기반 아웃박스](../architecture/event-driven-outbox.md)를 따른다.
