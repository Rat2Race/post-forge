...
게시글 상세 조회는 자주 호출되고, 매 요청마다 RDB `posts.view_count`를 즉시 update하면 write amplification과 row contention이 생긴다.
또 같은 사용자의 반복 조회는 24시간 안에 한 번만 카운트해야 한다.

## 결정

조회수는 Redis를 write buffer로 사용한다.
`post:views:{postId}`에 현재 조회수를 두고, `post:viewed:{postId}:{accountId}` guard key로 중복 조회를 막는다.
변경된 post id는 `post:views:dirty` set에 넣고, sync scheduler가 dirty set만 RDB로 반영한다.
...
- Redis 재시작 시 마지막 sync 이후 증가분은 유실될 수 있다. 현재 허용 손실 범위는 scheduler 주기 기준 최대 약 5분이다.
...
- `board/src/main/java/dev/iamrat/board/view/infrastructure/redis/ViewCountRedisKeys.java`
