...
외부 API의 로그인 식별자 이름은 `username`, 저장 column은 `accounts.user_id`다. 인증 이후 계정 식별과 소유권 판단은 `accounts.id` 기반 `accountId`를 기준으로 한다.

> 문서 경계: 이 문서는 계정 lifecycle policy만 정의한다. Auth endpoint/DTO/status는 `../api/README.md#auth`, 인증 runtime 구조는 `../architecture/authentication.md`를 따른다.
...
- 프로필 응답에는 각 endpoint DTO가 정의한 계정 식별자, 이메일, 닉네임, provider, 권한, 생성/수정 시각만 포함한다.
...
가입 후 기본 무료 plan과 private workspace를 만드는 동작도 이 target 범위에 포함한다.

## Error Rules

- 인증되지 않은 계정 조회/변경 요청은 `401 Unauthorized`로 처리한다.
- 본인이 아닌 계정 변경 요청은 `403 Forbidden`으로 처리한다.
- 중복 `username`, 이메일, 닉네임은 `409 Conflict` 의미로 처리한다.
