...
| `performance/` | 실행 가능한 성능 테스트 표면, 측정 결과, historical baseline | 현재 capacity가 아닌 과거 수치는 historical로 표시한다 |

## 구조

| 폴더 | 용도 |
| --- | --- |
| `architecture/` | 현재 시스템이 어떻게 동작하는지, 설계 경계와 흐름 정리 |
| `api/` | 모듈별 HTTP API 명세와 공통 요청/응답 규칙 |
| `decisions/` | 왜 그런 결정을 했는지 남기는 ADR |
| `performance/` | 성능 측정 결과, 병목 분석, capacity/cost 근거 |
...
- 파일명은 소문자 kebab-case를 기본으로 한다. 예: `redis-cache-strategy.md`
