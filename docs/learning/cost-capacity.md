...
> 분류: 학습·계산 참고자료. 공식과 historical input을 사용한 예시이며 포트폴리오 성능 주장이나 현재 prod capacity의 근거가 아니다.

이 문서는 요청량, 서버 부하, 클라우드 비용, 전기요금을 계산하는 방법을 설명한다. 클라우드 단가와 전기요금은 바뀔 수 있으므로 실제 견적에는 최신 가격표와 청구서를 다시 확인한다.
...
- 현재 prod host snapshot의 정본은 [운영 서버 스펙](../performance/prod-environment-spec.md)이다. 이 문서에 값을 복사하지 않는다.
- 현재 Intel N100 prod의 steady-state RPS, p95, CPU 상한은 아직 측정되지 않았다.
- 아래 수치 예시는 과거 Oracle Cloud ARM 1vCPU / 1GB 결과를 입력으로 사용한다.
...
## Historical 계산 부록: Oracle ARM 1vCPU / 1GB

이 부록의 원본 수치는 [k6 시나리오 결과](../performance/k6-scenario-results.md)와 [N+1 분석](../performance/n-plus-one-analysis.md)에 있다. 계산 방식을 보여주기 위한 고정 input이며 현재 운영 권장치가 아니다.

| historical 시나리오 | 처리량 | CPU/Load | 응답시간 |
|---|---:|---|---|
| 로그인 동일 비중 혼합 | 14.66 RPS | CPU 100%, Load 12.0 | 로그인 p95 3.52s |
| 읽기 25 VU + 로그인 5 VU | 23.75 RPS | CPU max 59%, Load 0.7 | `GET /posts` p95 508ms |
| batch size 적용 후 | 24.91 RPS | CPU 미관측 | 전체 p95 180.75ms |
...
- 이 과거 1GB VM 계산에서는 메모리보다 CPU와 로그인 비율의 영향이 컸다.

### 과거 1GB VM 트래픽 예시

| 목표 트래픽 | 판단 |
|---:|---|
| 1~5 RPS | 과거 A1 1 OCPU/1GB 계산에서 Always Free 범위 |
| 10~20 RPS | 과거 혼합 시나리오의 보수적 구간 |
| 25 RPS | 과거 관측 상단. 현재 prod 권장치가 아님 |
| 30~35 RPS | 과거 CPU 비용으로 환산한 이론 구간 |
| 50 RPS 이상 | 과거 1 OCPU/1GB 계산 범위 밖 |
| 로그인 5 RPS 이상 | 과거 환경에서 BCrypt CPU 병목 가능성이 컸음 |
...
이 문서의 1 OCPU / 1GB 수치는 계산 예시다. 현재 prod 수용량은 별도 성능 실험과 같은 시간창의 Prometheus metric을 수집한 뒤 이 공식으로 다시 계산한다.
