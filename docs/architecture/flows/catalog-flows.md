...
Endpoint 명세는 [통합 API 명세의 Catalog](../../api/README.md#catalog)가 canonical이다.

원리: 외부 source(Naver 등)마다 상품 식별자와 이름이 다르므로, `products`를 조회 모델로 두고 source별 외부 판매 항목은 `offers`로 분리한다. upsert 기준은 `(source, external_product_id)` unique다. 현재 모델은 여러 source를 완전히 병합한 canonical truth까지 보장하지 않는다.
...
현재 approve는 검토 결과만 기록하고 실제 product 병합이나 offer 이동을 수행하지 않는다. 또한 cross-source 자동 매칭 시 `Product.source/externalProductId`는 기존 source 값인데 `currentPrice/mallName`은 새 source 값으로 바뀔 수 있고, `Offer`에는 가격이 없어 어느 source의 현재가인지 온전히 표현하지 못한다. 이 제약을 해결하기 전에는 matching을 canonical 상품 정규화 완료로 해석하지 않는다.
