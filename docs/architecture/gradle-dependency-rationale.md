...
| `implementation project(':auth')`, `:board`, `:ai`, `:ingest`, `:collector`, `:ops` | 기능/운영 module을 하나의 monolith runtime에 조립한다. `app`이 module composition을 담당한다는 뜻이다. |
...
| `testImplementation spring-boot-starter-test` | app route/OpenAPI 조립 정책의 단위 테스트에 필요하다. |
...
Test Console처럼 실행 app에 붙는 local-only 도구도 구현은 `ops` module이 소유하고, `app`은 runtime에 조립만 한다.
Route/security 문자열은 `PostForgeSecurityRoutes`와 `PostForgeOpenApiRoutes`로 분리하고, OpenAPI security requirement는 feature package prefix가 아니라 `@PreAuthorize`와 `OpenApiSecurityPolicy` metadata로 판단한다.

기능 모듈의 error code와 OpenAPI group 설정은 `common`으로 모으지 않고 `<module>.support.error`, `<module>.support.openapi`에 둔다.
...
`NewsDocumentMetadata`는 collector가 만든 뉴스 문서를 ingest가 자동 게시 후보로 해석하기 위한 typed metadata 계약이다.
문자열 key 자체는 core 안에 묶고, collector/ingest는 public factory/parser만 사용한다.
...
| `springdoc-openapi-starter-webmvc-ui` | OpenAPI metadata와 security scheme 설정을 제공한다. |
...
S3는 `board`, OpenAI는 `ai`, OAuth/Gmail은 `auth`, Naver API는 `collector`가 소유한다.
`support`는 Spring infrastructure glue만 맡는다.

## `messaging/build.gradle`

`messaging`은 공통 outbox infrastructure module이다.
현재는 독립 module로 먼저 고정하고, 후속 phase에서 도메인 module이 쓰기 트랜잭션 안에서 event envelope을 저장할 수 있게 한다.
...
`CommentCreated`, `CollectedItemCreated` 같은 이벤트 타입과 발행 시점은 각 도메인 module이 결정하고, `messaging`은 저장/상태/재시도/전달 계약만 맡는다.
...
`ingest`는 문서 적재 API와 internal collector ingest orchestration을 소유한다.
...
| `spring-boot-starter-web` | `/ingest/documents`, `/internal/collector/documents` API controller를 제공한다. |
...
## `collector/build.gradle`

`collector`는 외부 source API 호출, source별 parsing, 수집 중복 판단을 소유한다.
현재는 app 안에 조립되는 library module이므로 자체 Spring Boot application, Dockerfile, Gradle wrapper를 갖지 않는다.

| 의존성 | 이유 |
|--------|------|
| `implementation project(':core')` | 수집 문서를 ingest로 넘기는 `SourceDocumentIngestor` port를 사용한다. |
| `spring-boot-starter-web` | Naver API 호출용 `RestClient`와 수동 trigger controller를 제공한다. |
| `spring-boot-starter-data-jpa` | 수집 기록과 source 중복 확인 repository를 소유한다. |
| `testImplementation spring-boot-starter-test` | collector module unit test에 필요하다. |

## `ops/build.gradle`

`ops`는 제품 도메인이 아니라 로컬 운영/테스트 도구를 소유한다.
현재 Test Console은 app에 조립되지만, 구현은 app composition root 밖에 둔다.

| 의존성 | 이유 |
|--------|------|
| `java-library` plugin | 실행 app이 아닌 library module로 조립된다. |
| `spring-boot-starter-web` | local-only console page/API controller와 정적 리소스 제공에 필요하다. |
| `testImplementation spring-boot-starter-test` | controller/service 단위 테스트와 MockMvc 검증에 필요하다. |

`ops`에 넣으면 안 되는 것은 다음이다.

- 제품 도메인 로직
- 인증/게시판/AI/수집 정책 구현
- 외부 운영 플랫폼 연동을 위한 무거운 dependency

`ops`는 쉽게 제거하거나 비활성화할 수 있는 운영 보조 module이어야 한다.
...
collector와 ingest 사이의 `NewsDocumentMetadata`도 HTTP request schema 또는 message payload schema로 승격할 수 있는 후보 계약이다.
...
원칙은 단순하다.
의존성은 "쓰는 곳"에 둔다.
계약은 `core`, Spring infrastructure는 `support`, 실행 조립은 `app`, 운영 보조 도구는 `ops`, 외부 adapter는 해당 feature module이 소유한다.
