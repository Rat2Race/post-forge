...
- `app`은 실행 조립 모듈이다. 소스는 `ApplicationServer` 중심으로 유지하고, runtime composition에 필요한 의존성은 `app/build.gradle`에 둔다.

## 현재 모듈 책임

| 모듈 | 책임 | 직접 선언해야 하는 주요 기술 |
|------|------|------------------------------|
| `app` | 실행 진입점, 전체 모듈 조립, JPA repository/entity scan bootstrap | Spring Web, Actuator, Spring Data JPA, Prometheus, PostgreSQL runtime, `support` |
| `core` | 공통 DTO, 공통 예외 타입, 공통 security principal 계약, 모듈 간 board/AI port 계약 | Spring Web API, Spring Data Commons, Jackson annotations |
...
| `ingest` | 문서 적재 API, internal crawl ingest, 자동 게시 orchestration | Web, Validation, Spring TX, Spring AI VectorStore API |
...
| `dev.iamrat.support.openapi.OpenApiConfig` | OpenAPI metadata, group, security customizer 구성 |
...
```bash
bash ./gradlew :core:compileJava :support:compileJava :auth:compileJava :board:compileJava :ai:compileJava :ingest:compileJava :app:compileJava
bash ./gradlew :core:test :support:test :auth:test :board:test :ai:test :ingest:test :app:test :app:bootJar
```
