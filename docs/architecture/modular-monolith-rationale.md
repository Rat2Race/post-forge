...
- `auth`, `board`, `ai`, `ingest`는 독립 기능 모듈로 두어 각 모듈이 자기 기술 의존성을 직접 선언한다.
...
| 기능 모듈별 기술 의존성 소유 | `auth/build.gradle`, `board/build.gradle`, `ai/build.gradle`, `ingest/build.gradle` | 모듈 하나를 떼어낼 때 필요한 framework/storage/client 의존성을 식별하기 쉽다. |
...
3. `ingest` service 분리
   - `crawl`이 현재 호출하는 internal ingest API를 독립 ingest endpoint로 옮긴다.
   - 문서 저장 후 자동 게시 요청은 AI service API 또는 message로 전달한다.
   - PgVector ownership은 분리 시점에 결정해야 한다. 권장 방향은 AI/RAG service가 vector store를 소유하고 ingest는 문서 적재 요청만 보내는 구조다.

4. `auth` service 분리
   - `members`, `member_roles`, refresh token/email verification Redis key를 auth가 소유한다.
...
- PgVector를 AI/RAG가 소유할지 ingest가 소유할지에 대한 최종 결정
