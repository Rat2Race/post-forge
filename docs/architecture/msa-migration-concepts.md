...
|---------------|------------------|------|
| `auth` | `accounts`, `account_roles`, refresh token/email verification Redis key | 계정 identity, credential, token state는 인증 도메인이다. |
...
| `ingest` | ingest request log, collector ingest state, deduplication key | 외부 수집 요청의 수신/중복/처리 상태는 적재 도메인이다. |
...
| `aggregate_type` | `post`, `document`, `account` 같은 도메인 대상 |
| `aggregate_id` | 대상 id |
...
예를 들어 collector가 문서를 보냈고, ingest가 AI 분석을 요청했고, AI가 board에 게시글을 생성했다면 이 전체 흐름을 하나의 trace로 묶어야 한다.
...
PostForge의 현재 ownership decision은 Option A다.
...
현재 테이블/저장소 소유권 표는 [DB Schema Ownership](../db/schema-ownership.md)에 둔다.
...
3. First extraction
   - 가장 독립적인 흐름 하나를 고른다.
   - 추천 후보는 `collector`처럼 외부 API/비동기 처리 성격이 강한 영역 또는 `ai/rag`처럼 독립 부하와 장애 경계가 분명한 영역이다.
