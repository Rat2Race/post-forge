...
| Redis-backed auth state | `auth` | refresh token, email verification, OAuth2 exchange code key ownership |
...
- 이메일 인증과 OAuth2 exchange code는 Redis TTL state로 관리한다.
- OAuth2, refresh token, 이메일 인증 Redis state 장애는 인증 안전성을 위해 fail-closed로 처리한다.
...
2. Spring Security authentication manager가 username/password를 검증한다.
...
- Redis 장애 시 refresh reissue, OAuth2 exchange, 이메일 인증이 실패할 수 있다.
