...
export function EditPostPage() {
  ...
  const [attachments, setAttachments] = useState<AttachmentPillProps[]>([
    { id: "1", fileName: "삼성전자_1Q26_실적발표자료.pdf", fileType: "pdf", fileSize: 2450000, url: "#" },
  ]);
  ...
  const handleFileUpload = (files: FileList | null) => {
    ...
    Array.from(files).forEach((file) => {
      if (!allowedTypes.includes(file.type)) { alert(`${file.name}: 지원하지 않는 파일 형식입니다.`); return; }
      if (file.size > maxSize) { alert(`${file.name}: 파일 크기가 10MB를 초과합니다.`); return; }
  ...
  return (
    <div className="px-5 lg:px-8 py-8 pb-16">
      <div className="max-w-3xl mx-auto">
        <div className="mb-6">
          <h1 className="serif-headline text-2xl mb-1">글 수정</h1>
          <p className="text-sm text-muted-foreground">게시글 내용을 수정할 수 있습니다</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <Textarea id="summary" value={formData.summary} onChange={(e) => setFormData({ ...formData, summary: e.target.value })} className="min-h-20" required />
          </div>
          ...
          <div>
            ...
            <Textarea id="content" value={formData.content} onChange={(e) => setFormData({ ...formData, content: e.target.value })} className="min-h-80 font-mono text-sm" required />
          </div>
              ...
              <div className="mb-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {attachments.map((attachment) => (
                    <AttachmentPill key={attachment.id} {...attachment} onRemove={() => setAttachments(prev => prev.filter(att => att.id !== attachment.id))} />
                  ))}
                </div>
              </div>
            )}
            <div className="border-2 border-dashed border-border/60 rounded-lg p-5 text-center hover:border-border transition-colors">
          ...
          <div className="flex gap-3 justify-end pt-2 border-t border-border/40">
            <Button type="button" variant="outline" onClick={() => navigate(`/posts/${id}`)}>취소</Button>
