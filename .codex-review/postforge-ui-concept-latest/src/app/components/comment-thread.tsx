...
export function CommentThread({ comments, postAuthorIsAI, onAddComment, onAskAIReply }: CommentThreadProps) {
  ...
  return (
    <div className="space-y-6">
      {/* New Comment Form */}
      <div className="bg-secondary/30 rounded-lg border border-border/40 p-5">
        <h3 className="text-sm mb-3">댓글 작성</h3>
        <Textarea
          ...
          <div key={comment.id} className="space-y-4">
            ...
            <div className="flex gap-4">
              <Avatar className="h-10 w-10 shrink-0">
                <AvatarFallback className={comment.isAIGenerated ? "bg-secondary text-muted-foreground" : ""}>
                    ...
                    {comment.replies.map((reply) => (
                      <div key={reply.id} className="flex gap-3">
                        <Avatar className="h-8 w-8 shrink-0">
                          <AvatarFallback className={reply.isAIGenerated ? "bg-secondary text-muted-foreground" : ""}>
