...
const METHOD_ORDER = ["GET", "HEAD", "OPTIONS", "POST", "PUT", "PATCH", "DELETE", "ANY"];
...
function writeResult(result, discoveredCount, existingCount) {
...
function appendReport(result, discoveredCount, existingCount) {
  ...
  const report = [
    ...
    "",
    "Policy sync preserves existing endpoint classes. New write routes are scenario candidates, path-variable reads are draft candidates, and manual/internal routes require review.",
    "",
  ].join("\n");
...
const discoveredEndpoints = discoverEndpoints(inventory);
const result = mergePolicy(existingEndpoints, discoveredEndpoints);

writePolicy(policyText, result.merged);
writeResult(result, discoveredEndpoints.length, existingEndpoints.length);
appendReport(result, discoveredEndpoints.length, existingEndpoints.length);
