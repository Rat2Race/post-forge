...
앱 서버가 실행 중이면 Bruno/k6 smoke를 자동 실행하고 결과 report를 만든다. setup의 기본 `baseUrl`은 `http://localhost:8080`으로 고정한다.
...
`setup/state/*.json`과 `setup/reports/*.md`도 실행 기록과 보고서라 commit 대상이 아니다. 반대로 `setup/`의 executor, manifest, tool module 파일은 setup 자체의 관리 표면이므로, 기존 작업트리 상태가 변경/삭제/추가로 보이더라도 agent가 임의로 복구하거나 정리하지 않는다.
