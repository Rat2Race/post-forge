...
| `testing-fixtures.json` | 선택적 프로젝트별 fixture override. setup 엔진 산출물이 아니며 repo root에 둘 수 있다. |
...
| SQL/auth fixture | `setup/run.sh generate-tests`는 기본적으로 runtime source roots의 모든 `@Entity` + `@Id`를 분석해 conservative SQL seed를 생성한다. `testing-fixtures.json`이 있으면 SQL/auth fixture override로 사용한다. setup 엔진에 특정 프로젝트 SQL을 하드코딩하지 않는다. |
...
| `testing-fixtures.json` visible | 의도된 상태다. 기본 entity-derived seed/auth 추론으로 부족한 프로젝트에서만 쓰는 선택적 override다. |
...
| generated 테스트/SQL fixture 생성 | `setup/run.sh generate-tests` |
...
| 6 | `./setup/run.sh generate-tests` | Bruno/k6 generated 산출물과 SQL fixture 생성 | `setup/state/generated-tests-result.json` |
...
| 4 | `testing-fixtures.json` | 기본 entity-derived seed/auth 추론으로 부족할 때만 프로젝트별 override 작성 | optional fixture override |
| 5 | `./setup/run.sh generate-tests` | policy 기준 Bruno/k6 generated 산출물과 JPA entity-derived SQL/auth fixture 생성 | `setup/state/generated-tests-result.json` |
| 6 | `tests/bruno/api/generated` | policy 기준 Bruno generated request 생성 | generated collection |
| 7 | `tests/k6/env.js`, `tests/k6/report.js`, `tests/k6/generated` | k6 기본 환경 모듈, 리포트 helper, policy 기준 k6 smoke script 생성 | env module, report helper, generated script |
| 8 | `tests/sql`, `tests/smoke/seed.sh` | entity-derived 또는 fixture override 기반 SQL fixture와 DB seed helper 생성 | fixture scripts |
| 9 | `setup/reports/testing-setup-report.md` | 낮은 confidence, reviewRequired, 보류 사유 정리 | report |
...
| 새 generated 파일 생성 | 낮음 | `generated/` 아래 또는 manifest에 선언된 SQL fixture 경로에만 생성 |
...
| generated 테스트 재생성 | 낮음 | `generated/` 영역과 fixture config가 선언한 SQL/auth fixture만 재생성하고 `manual/`은 건드리지 않음 |
