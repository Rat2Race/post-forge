    ...
    public ProductSourceResult search(ProductSourceQuery query) {
        ...
        try {
            ...
            List<ProductSourceItem> items = response == null ? List.of() : response.items().stream()
                ...
                .toList();
            log.info(
                "Naver Shopping 검색 완료. keyword={}, requestedDisplay={}, itemCount={}",
                query.keyword(),
                query.displayCount(),
                items.size()
            );
            if (log.isDebugEnabled() && !items.isEmpty()) {
                ProductSourceItem firstItem = items.getFirst();
                log.debug(
                    "Naver Shopping 첫 결과. keyword={}, externalProductId={}, title={}, price={}, mallName={}",
                    query.keyword(),
                    firstItem.externalProductId(),
                    firstItem.title(),
                    firstItem.price(),
                    firstItem.mallName()
                );
            ...
            log.warn(
                "Naver Shopping 검색 실패. keyword={}, requestedDisplay={}, status={}, reason={}",
                query.keyword(),
                query.displayCount(),
