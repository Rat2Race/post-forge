...
│   └── src/main/java/.../
│       ├── document/             # /ai/documents 문서 저장 API/service
...
│       ├── global/exception/     # CustomException, ErrorCode, ErrorResponse
│       ├── global/security/      # 공통 principal 계약
│       └── board/                # PostWriter/Reader 등 모듈 간 port 계약
│
...
```text
app     → support, auth, board, ai, ingest, core
support → core (+ redis config, springdoc-openapi, global web exception handler)
auth   → core (+ web, validation, jpa, redis, security, jwt, mail, oauth2)
board  → core (+ web, validation, jpa, redis, security, aws-s3)
ai     → core (+ web, validation, jdbc, spring-ai-openai, spring-ai-pgvector)
ingest → core (+ web, validation, spring-tx, spring-ai-vector-store)
core   → spring-web API, spring-data-commons API, jackson-annotations

crawl  → 독립 Spring Boot 애플리케이션. 메인 app에는 /internal/crawl/documents로 REST 전송
```
...
- SpringDoc OpenAPI 전역 설정을 `support` 모듈에 배치
...
- `auth`, `board`, `ai`, `internal`, `all` 그룹 문서 제공
...
| `POST` | `/ai/documents` | USER, ADMIN | 벡터 DB 문서 저장 |
| `POST` | `/internal/crawl/documents` | Internal API Key 또는 인증된 관리자 | 크롤러 문서 적재 + 자동 게시 후보 처리 |
| `POST` | `:8090/crawl/naver-news` | crawl 서비스 | 네이버 뉴스 수동 크롤링 트리거 |

---

## API 문서

SpringDoc OpenAPI 전역 설정은 `support` 모듈에서 관리합니다.
