...
> 외부 쇼핑 API 기반 상품 큐레이션 커머스 커뮤니티 백엔드

PostForge는 외부 쇼핑 API 데이터를 수집해 상품 카탈로그와 가격 이력을 만들고, 그 위에 상품 중심 게시판과 커뮤니티 기능을 얹는 커머스 커뮤니티 백엔드입니다. 장기적으로는 AI/system이 수집 상품 정보를 바탕으로 자동 게시글을 만들고, 회원은 상품별 게시판에서 직접 글과 댓글을 남기며, 유료 회원은 원하는 상품이나 카테고리를 AI로 검색/분석하는 흐름을 목표로 합니다.

현재 구현의 중심은 **운영 가능한 게시판 백엔드 토대**와 **외부 상품 수집 -> catalog 정규화 -> price snapshot/read model** 흐름입니다. 목표 제품상과 현재 구현 범위는 [Product Direction](./docs/product/README.md)에 분리해 정리합니다.
...
| Product Direction | Roadmap / In progress | product boards, AI/system-authored posts, paid AI-assisted product search, free generated-content access |
...
Manual k6 scenarios live under [`k6/`](./k6/README.md). Run results are inspected in
Prometheus/Grafana rather than through an in-app Test Console.
...
See [Product Direction](./docs/product/README.md) for the canonical product-facing plan. This README keeps the portfolio entry point concise and separates shipped backend foundation from the future product shape.
...
Current implementation is backend-oriented:

- `board` provides posts, comments, likes, files, and view-count foundations.
- `source`, `ingest`, `catalog`, and `price` support product collection, normalization, snapshots, and read models.
- `ai` provides AI/RAG foundation and draft-generation boundaries.
- `board` stores AI/system draft and product-post link data through `auto_post_drafts` and `post_product_links`.

### Future Product Direction

The target product is a category-based product community:

- Product/category boards become the main reading and discussion surface.
- AI/system-authored posts are visibly distinct from member-authored posts.
- External product information collection feeds automatic product posts and price-tracking views.
- Free members can use normal community features and access generated product content.
- Paid members can run on-demand AI-assisted product or category search, comparison, and custom information gathering.
...
Access and membership rules are documented in [Access Policy](./docs/policy/access-policy.md).
...
| --- | --- |
| [Product Direction](./docs/product/README.md) | product concept, current implementation, future direction, authorship model, membership access |
...
| [Modular Monolith Rationale](./docs/architecture/modular-monolith-rationale.md) | module boundary and modular-monolith tradeoffs |
| [MSA Migration Concepts](./docs/architecture/msa-migration-concepts.md) | future service split, messaging, DB ownership, API versioning |
