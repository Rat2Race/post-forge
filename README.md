...
| **External API** | DART Open API (공시/재무), 네이버 검색 API (뉴스) |
...
### AI 게시글 자동 생성 (RAG)
- DART 공시 + 재무 수치 + 네이버 뉴스를 벡터 DB에 저장 후 유사도 검색
- 종목코드 기반으로 관련 데이터를 수집하여 LLM에 전달 (GPT-4o-mini)
- 시스템 프롬프트로 주식 시장 분석가 페르소나 적용 (투자 권유 금지, 출처 인용 필수)
- JSON 형식 응답 파싱 (title, summary, content, tags)
- `PostWriter` 인터페이스를 통한 Board 모듈 연동 (게시글 자동 등록)
- 미리보기(publish=false) / 즉시 등록(publish=true) 모드 지원

### 데이터 크롤링 (crawl 모듈)
- DART Open API: 주요사항보고(B), 정기공시(A), 지분공시(D) 자동 수집
- DART 재무 수치 API: 정기공시 감지 시 매출액/영업이익/당기순이익 등 자동 수집
- 네이버 검색 API: 키워드 기반 뉴스 기사 자동 수집
- 중복 방지: 이미 크롤링한 공시/기사는 DB 조회로 필터링
- AI 우선 전송: 벡터 DB 전송 성공 후에만 크롤링 이력 저장 (데이터 유실 방지)
- 정기공시 크롤링 시 게시글 자동 생성 트리거 (crawl → ai REST 호출)
- 스케줄러 기반 주기적 크롤링 + 수동 트리거 API 지원
...
| `POST` | `/ai/posts/generate` | AI 게시글 생성 (publish: true/false) |
...
| `POST` | `/crawl/dart` | DART 공시 수동 크롤링 트리거 |
