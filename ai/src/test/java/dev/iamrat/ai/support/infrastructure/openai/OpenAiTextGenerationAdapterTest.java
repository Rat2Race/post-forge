...
class OpenAiTextGenerationAdapterTest {
    ...
    @Test
    @DisplayName("시스템과 사용자 프롬프트를 Spring AI Prompt로 변환해 ChatModel을 호출한다")
    void generate_buildsSpringAiPrompt() {
        OpenAiTextGenerationAdapter adapter = adapter("ollama");
    ...
    void generate_whenOpenAiSucceeds_recordsSuccessMetrics() {
        OpenAiTextGenerationAdapter adapter = adapter("ollama");
        given(chatModel.call(any(Prompt.class))).willReturn(chatResponse("응답", 10, 3));
    ...
    void generate_whenOpenAiUnavailable_returnsNull() {
        OpenAiTextGenerationAdapter adapter = adapter("openai");
    ...
    void generate_whenOpenAiUnavailable_recordsDegradedMetrics() {
        OpenAiTextGenerationAdapter adapter = adapter("openai");
        willThrow(new IllegalStateException("quota"))
            .given(chatModel)
            .call(any(Prompt.class));
        ...
        assertThat(meterRegistry.counter("ai_text_generation_degraded_total", "provider", "openai").count())
            .isEqualTo(1.0);
    ...
    private OpenAiTextGenerationAdapter adapter(String provider) {
        return new OpenAiTextGenerationAdapter(
            chatModel,
            metrics(provider)
        );
    }

    private OpenAiTextGenerationMetrics metrics(String provider) {
        return new OpenAiTextGenerationMetrics(meterRegistry, openAiProperties(provider));
    }

    private OpenAiProperties openAiProperties(String provider) {
