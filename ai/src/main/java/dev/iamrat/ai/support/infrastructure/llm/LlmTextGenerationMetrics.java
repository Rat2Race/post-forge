...
/*
 * LLM ChatModel 기반 텍스트 생성 호출에서 발생하는 관측 지표를 기록하는 컴포넌트입니다.
 *
 * LlmTextGenerationAdapter는 프롬프트를 만들고 ChatModel을 호출하는 책임만 가지도록 두고,
 ...
 * - 프롬프트/응답 문자 수
 * - ChatModel metadata가 제공하는 prompt/completion/total token 사용량
 */
@Slf4j
@Component
...
public class LlmTextGenerationMetrics {
     ...
     *
     * 응답 문자 수는 항상 기록하고, token usage는 ChatModel 응답 메타데이터에 존재하는 경우에만 기록합니다.
     * 로컬 모델이나 일부 LLM gateway는 token usage를 제공하지 않을 수 있으므로 해당 값은 선택적으로 다룹니다.
     */
    ...
    /*
     * ChatModel 응답 메타데이터에 포함된 token usage를 summary 지표로 기록합니다.
     *
