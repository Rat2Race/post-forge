...
class AutoPostOrchestratorTest {
    ...
    void publishEligible_newsDocuments_publishOncePerLink() {
        DocumentRequest first = request(autoPostMetadata("테크", "AI 반도체 수요 증가", "https://news.example/1"));
        DocumentRequest duplicate = request(autoPostMetadata("테크", "AI 반도체 수요 증가", "https://news.example/1"));
    ...
    void publishEligible_withoutOptIn_skipsDocument() {
        DocumentRequest request = request(new NewsDocumentMetadata(
            "테크",
            "AI 반도체 수요 증가",
    ...
    void publishEligible_failureContinuesWithNextDocument() {
        DocumentRequest failing = request(autoPostMetadata("테크", "AI 반도체 수요 증가", "https://news.example/1"));
        DocumentRequest succeeding = request(autoPostMetadata("정책", "플랫폼 규제 완화", "https://news.example/2"));
    ...
    private DocumentRequest request(Map<String, String> metadata) {
        return new DocumentRequest("content", NewsDocumentMetadata.SOURCE_NAVER_NEWS, metadata);
    }
