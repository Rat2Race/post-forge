...
class LaunchNewsEligibilityPolicyTest {
    ...
    @Test
    @DisplayName("블로그 호스트는 신뢰할 수 없는 출처로 거절한다")
    void rejectsUntrustedHostMarker() {
        assertThat(policy.evaluate(candidate(
            "갤럭시북",
            "갤럭시북 신제품 출시",
            "삼성이 갤럭시북 신제품을 공개했다.",
    ...
    @Test
    @DisplayName("점이 없는 호스트는 거절한다")
    void rejectsSourceWithoutDot() {
        assertThat(policy.evaluate(candidate(
            "갤럭시북",
            "갤럭시북 신제품 출시",
            "삼성이 갤럭시북 신제품을 공개했다.",
    ...
    @Test
    @DisplayName("구독 키워드가 있어도 출시 단어가 없으면 거절한다")
    void rejectsWhenLaunchWordAbsentEvenWithKeyword() {
        assertThat(policy.evaluate(candidate(
            "갤럭시북",
            "갤럭시북 노트북 판매량 증가",
            "삼성이 갤럭시북 판매량을 늘렸다.",
